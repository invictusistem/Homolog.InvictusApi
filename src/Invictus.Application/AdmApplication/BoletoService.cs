...
namespace Invictus.Application.AdmApplication
{
    ...
    public class BoletoService : IBoletoService
    {
        ...
        public async Task<BoletoLoteResponse> GerarBoleto(decimal valor, decimal valorBonusPontualidade, DateTime vencimento, DadosPessoaDto pessoa, int numeroPedido)
        {
            ...
            //var boletosResponse = new List<BoletoLoteResponse>();
            var boletoResp = new BoletoLoteResponse();
            //int qntBoletosSalvos = _db.LogBoletos.Count();
            ...
            NumberFormatInfo config = new NumberFormatInfo();
                ...
                parametross.Add("instrucoes", "");
                parametross.Add("instrucao_adicional", "");
                parametross.Add("grupo", "Boletos");
                parametross.Add("webhook", "http://example.com.br");
                parametross.Add("pedido_numero", numeroPedido.ToString());
                parametross.Add("especie_documento", "DS");
                parametross.Add("pix", "pix-e-boleto");
                ...
                using (HttpClient client = new HttpClient())
                {
                    ...
                    boletoResp = JsonSerializer.Deserialize<BoletoLoteResponse>(retorno);
                    boletoResp.vencimento = vencimento;

                   // boletosResponse.Add(boletoResp);

                    _logger.LogInformation($"Boleto create at {DateTime.UtcNow.TimeOfDay}");
