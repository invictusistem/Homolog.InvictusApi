...
namespace Invictus.Api.Controllers
{
    public class TransacoesDto
    {
        ...
        {
            //this.id = new Guid(id);
            //this.tipo = TipoTransacao.FromValue<TipoTransacao>(Convert.ToInt32(this.tipo)).DisplayName;
        }
...
public class TestController : ControllerBase
{
    ...
    [HttpGet]
    [AllowAnonymous]
    [Route("TesteCodes-transacao")]
    public async Task<IActionResult> TesteTransacao()
    {
        /*

        var trans = new Transacao()
        {
            Tipo = TipoTransacao.CreditoVista
        };

        await _db.Transacoes.AddAsync(trans);

        await _db.SaveChangesAsync();
        */

        IEnumerable<TransacoesDto> transacs = new List<TransacoesDto>();
                ...
                _config.GetConnectionString("InvictusConnection")))
        {
            ...
            try
            {
                transacs = await connection.QueryAsync<TransacoesDto>(query);
            }
            catch (Exception ex)
            {
