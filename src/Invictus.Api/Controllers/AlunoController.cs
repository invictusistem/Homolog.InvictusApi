...
namespace Invictus.Api.Controllers
{
    ...
    //[Authorize]
    [Route("api/aluno")]
    public class AlunoController : MainController//Controller
    {
        ...
        public AlunoController(SignInManager<IdentityUser> signInManager, UserManager<IdentityUser> userManager,
        ...
        [HttpPost("registrar/{alunoId}/{senha}")]
        [AllowAnonymous]
        public async Task<IActionResult> Registrar(int alunoId, string senha)
        {
            // if (!ModelState.IsValid) return BadRequest();
            var aluno = await _db.Alunos.Where(a => a.Id == alunoId).FirstOrDefaultAsync();

            if (aluno.Email == null) return BadRequest(new { message = "O aluno precisa ter um emila cadastrado para se cadastrar." });

            var nomeAluno = aluno.Nome.Split(' ');
            var user = new IdentityUser
            {
                UserName = nomeAluno[0],
                Email = aluno.Email,
                EmailConfirmed = true
            ...
            if (result.Succeeded)
            {
                ...
                var unidade = _db.Unidades.Find(aluno.UnidadeCadastrada);
                await _userManager.AddToRoleAsync(user, "MasterAdm");
                await _userManager.AddClaimAsync(user, new System.Security.Claims.Claim("IsActive", true.ToString()));
